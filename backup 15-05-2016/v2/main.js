...
var statsFolder = 'statistics/'; 
var schedulesFolder = 'schedules/';
var statesFolder = 'switch-states/'
...
//switch state variables(they store integers representing ON or OFF)
var state1 = switch1Off; 
var state2 = switch2Off;
var state3 = switch3Off;
var state4 = switch4Off;

//create switch states persistance variables
var state1File = 'state1.txt'; 
var state2File = 'state2.txt';
var state3File = 'state3.txt';
var state4File = 'state4.txt';

//create states directory to persist schedules to file in case of a power cut
mkdirp(statesFolder, function(error) {
	if(error) {
		console.log(error);
	}
}); 
...
io.sockets.on('connection', function (socket) { //gets called whenever a client connects
	...
	//=============================================================================
	if(fileExists(statesFolder + state1File)) {
		var state1 = fs.readFileSync(statesFolder + state1File,'utf8');
	}
	
	if(fileExists(statesFolder + state2File)) {
		var state2 = fs.readFileSync(statesFolder + state2File,'utf8');
	}
	
	if(fileExists(statesFolder + state3File)) {
		var state3 = fs.readFileSync(statesFolder + state3File,'utf8');
	}
	
	if(fileExists(statesFolder + state4File)) {
		var state4 = fs.readFileSync(statesFolder + state4File,'utf8');
	}
...
//            serialPort.write(buf); //transmits the buffer to the arduino
			fs.writeFile(statesFolder + state1File, state1, function (err) {
...
//            serialPort.write(buf); //transmits the buffer to the arduino
			fs.writeFile(statesFolder + state2File, state2, function (err) {
  				if (err) return console.log(err);
			});
...
//            serialPort.write(buf); //transmits the buffer to the arduino
			fs.writeFile(statesFolder + state3File, state3, function (err) {
  				if (err) return console.log(err);
			});
...
//            serialPort.write(buf); //transmits the buffer to the arduino
			fs.writeFile(statesFolder + state4File, state4, function (err) {
				if (err) return console.log(err);
			});
...
//            serialPort.write(buf); //transmits the buffer to the arduino
	...
	setInterval(function () { 
		...
		io.sockets.emit('stream', {value1:sensorStream1, value2:sensorStream2, value3:sensorStream3, value4:sensorStream4}); //sends the updated 		 stream values to all connected clients
